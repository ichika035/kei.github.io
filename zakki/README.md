...
2. `/zakki.html` の `cat-field` にアイコンを足す (テンプレートは `zakki.html` の中のコメント)
   - アイコンは画像 1 枚だけ。文字は載せません (カテゴリ名は `title` / `aria-label` に書く)
   - 画像は `zakki/images/` に置き、正方形に近いものだと切れ方が自然です
   - アイコンは `zakki-drift.js` がページの中をゆっくり漂わせ、へりで向きを変えます
     (速さは `zakki-drift.js` の `SPEED` で調整)
